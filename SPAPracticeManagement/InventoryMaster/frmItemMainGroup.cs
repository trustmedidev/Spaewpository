...
{
    public partial class frmItemMainGroup : InventoryDashboard
    {
        #region === Variables
        Boolean falag = true;
        ...
        private void frmItemMainGroup_Load(object sender, EventArgs e)
        {
            CommonCL.PanelControlGotFocus(pnlTabControlSearch, pnlTabControlAdd);
            objFrmName = "Item Main Group";
            SirchGridFormat();
        }
        ...
        public override void SirchGridFormat()
        {
            try
            {
                objItemMainGrpDAL.BindList(grdSearch);
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            if (ValidateForm())
            {
                int x = objItemMainGrpDAL.Insert(txtName.Text.ToString(), GlobalCL.UserCD);
                if (x > 0)
                {
